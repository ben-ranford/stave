...
| `AC-21` | All proposed performance targets are measured or accepted via ADR | `make verify-performance`<br>`.artifacts/performance/report.json`<br>`scripts/rigor/refresh-generated.sh performance-report`<br>`adapters/ssh/bridge_test.go::TestSSHTransportPerformanceBudget`<br>`docs/adr/ADR-017.md` | implemented | M8 |
