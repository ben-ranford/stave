...
func TestBindSessionEnforcesTreeLimitBeforeAdvancingPatchBaseline(t *testing.T) {
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			checkBridgeTreeLimitRecovery(t, test.bindLimit, test.serverLimit, test.clientLimit)
		})
	}
}

func checkBridgeTreeLimitRecovery(t *testing.T, bindLimit, serverLimit, clientLimit int) {
	t.Helper()
	s := bridgeSessionWithTreeSize(t, 1)
	defer s.Close()
	bound, err := BindSession(s, Options{CompatibilityMode: true, MaxTreeNodes: bindLimit})
	if err != nil {
		t.Fatal(err)
	}
	bound.MaxTreeNodes = serverLimit
	server := New(bound)
	defer server.Close()
	...
	if response := request("stave.initialize", fmt.Sprintf(`{"limits":{"maxTreeNodes":%d}}`, clientLimit)); response.Error != nil {
